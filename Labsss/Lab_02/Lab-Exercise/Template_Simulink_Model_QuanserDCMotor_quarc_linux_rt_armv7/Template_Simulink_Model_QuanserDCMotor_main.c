...
typedef RT_MODEL_Template_Simulink_Mo_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Template_Simulink_Model_QuanserDCMotor(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Template_Simulink_Model_Quan_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Template_Simulink_Model_QuanserDCMotor -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = Template_Simulink_Model_QuanserDCMotor();
  if (rtmGetErrorStatus(S) != NULL) {
